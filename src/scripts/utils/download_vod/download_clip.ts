import type { PlayBackURL, Clip, ContentInfo } from '../../types/vod'

export function isClipPage (): boolean {
  return location.pathname.startsWith('/embed/clip')
}
...
export async function download (url: string, title: string): Promise<void> {
  await chrome.storage.local.set({ clipDownloadURL: url, title })
  window.open(chrome.runtime.getURL('pages/download_clip.html'))
}
